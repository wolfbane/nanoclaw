 ...
 *
 * Adding a new provider's host config: create `src/providers/<name>.ts` with
 * a top-level `registerProviderContainerConfig(...)` call, then add
...
export interface ProviderContainerContext {
  /** Per-session host directory: `<DATA_DIR>/sessions/<group_folder>`. */
  sessionDir: string;
  /** Agent group folder name, for any per-group logic. */
  agentGroupId: string;

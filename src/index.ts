...
const pipelineDeps: import('./message-pipeline.js').PipelineDeps = {
    ...
    try {
      ...
      if (messages.length > 0) {
        ...
        for (const [chatJid, groupMessages] of messagesByGroup) {
          ...
          // Pull all messages since the agent cursor so non-trigger
...
async function main(): Promise<void> {
  ...
  startSchedulerLoop({
    ...
    sendMessage: async (jid, rawText, source) => {
      ...
      if (text) await channel.sendMessage(jid, text, { source });
    },
  });
  startIpcWatcher({
    sendMessage: (jid, rawText, source) => {
      const channel = findChannel(channels, jid);
      if (!channel) throw new Error(`No channel for JID: ${jid}`);
      const text = formatOutbound(rawText, channel.name as ChannelType);
      if (!text) return Promise.resolve();
      return channel.sendMessage(jid, text, { source });
    },
    registeredGroups: () => registeredGroups,
    registerGroup,
  ...
  startSessionCleanup();
  queue.setProcessMessagesFn((jid) => processGroupMessages(jid, pipelineDeps));

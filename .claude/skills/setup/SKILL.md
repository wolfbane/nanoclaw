...
Credentials are configured in `.env` at the project root. The built-in credential proxy injects them at request time so they never enter the container.
...
- CREDENTIALS=missing → re-run step 4 (check `.env` for credentials)
...
**Service not starting:** Check `logs/nanoclaw.error.log`. Common: wrong Node path (re-run step 7), missing credentials in `.env`, missing channel credentials (re-invoke channel skill).
